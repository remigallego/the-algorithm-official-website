...
const LogoSvg: FunctionComponent<Props> = (props) => {
  ...
  const [dashArray, setDashArray] = useState(80);
  ...
  const getSkewValues = () => {
    const skewX = (mouseX - width / 2) / 100;
    const skewY = (mouseY - height / 2) / 100;
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (dashArray >= 50) {
          setDashArray(randomInteger(6, 30));
        }
        if (dashArray < 50) {
          setDashArray(randomInteger(50, 120));
        }
      }}
    >
      <svg
        ...
        stroke="blue"
        strokeWidth="1"
        className={clicked ? "fill-animation" : ""}
      ...
      <style jsx>{`
        ...
        .path {
          ...
          animation: dash 9999s infinite linear, width 3s infinite alternate,
            color 100ms infinite alternate;
        }
